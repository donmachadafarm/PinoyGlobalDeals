	...
	$.extend($.fn.modal.Constructor.prototype, {
		hide: function (e) {
			...
			if(animate == 'animated'){
				...
				var that = this;
				this.$element.off('animateCssEnd').on('animateCssEnd',function(e){
					that.hideModal();
				});
				var effect = this.$element.attr('data-animate-css-hide') || 'fadeOutUp';
				this.$element.animateCss(effect);
			} else {
		...
		show: function (_relatedTarget) {
			...
			if(animate == 'animated'){
				...
				this.backdrop(function () {
					...
					that.$element.off('animateCssEnd').on('animateCssEnd',function(){
						that.$element.trigger('focus').trigger(e)
					});
					var effect = that.$element.attr('data-animate-css-show') || 'fadeInDown';
					that.$element.animateCss(effect);
				})
			} else {
				...
				if (this.isShown && this.options.backdrop) {
					...
					doAnimate ?
						this.$backdrop
							...
							.emulateTransitionEnd(that.BACKDROP_TRANSITION_DURATION)
